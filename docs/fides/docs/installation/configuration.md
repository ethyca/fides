...
=== "CLI Section"
    ...
    | analytics_id | String | "" | A cryptographically secure, fully anonymized string generated by `fideslog`. |
...
=== "User Section"
    ...
    | analytics_opt_out | Boolean | "" | When set to `False`, anonymous event data is collected via `fideslog`. |
