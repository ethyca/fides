...
A system can have multiple declarations.
 ...
 ```   json
 {
  ...
  //a fides key is an identifier label that must be unique within your
  //organizations systems. A fides key  can only contain alphanumeric
  //characters, '_', and '-'.
  "fidesKey":"bobco_service_api",

  // The version stamp is an incrementing number attached to your organization.
  // It tracks the current version attached to your organization as any action
  // is applied and can thus be used for "before/after" comparisons.
  // The version stamp is generated by the fides server.
  "versionStamp":123,

  // A freeform convenience identifier
  "fidesSystemType":"API",
  "name":"Bob",
  "description":"Bobco's service API",

  // A list of privacy declarations. Each named declaration shows
  // - the categories it applies to (multiple values)
  // - the uses to which that data is being put (single value)
  // - qualification by level of personalization (single value)
  // - the subjects of this data (multiple values)
  "declarations":[
    {
      "dataCategories":[
  ...
  // systems that this system makes use of
  "systemDependencies":[
    "test_system_2",
    "test_system_1"
  ],

  //datasets that this system makes use of
  "datasets":[
    "test-dataset"
  ],
