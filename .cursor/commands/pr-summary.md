...
- If `gh` is available, use `gh pr create -d --title "[title]" --body "[body]"` cli command to create the PR in draft mode
- If `gh` is not available, provide a link to a GitHub URL with the body of the PR template and title as query parameters. The body should be in Markdown code format, patterned after the template.
