...
from .mailchimp_connector import create_mailchimp_connector
from .mongodb_connector import create_mongodb_connector
from .postgres_connector import create_postgres_connector
from .privacy_request import create_privacy_request
from .s3_storage import create_s3_storage
from .stripe_connector import create_stripe_connector
from .user import create_user


def initiate_example_data_loading() -> None:
    """Main execution path for loading example data."""
    ...
    if get_secret("MAILCHIMP_SECRETS")["api_key"]:
        print(
            "Mailchimp secrets provided, attempting to configure Mailchimp connector..."
        )
