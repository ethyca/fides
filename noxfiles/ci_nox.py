...
def _check_test_directory_coverage(test_dir: str) -> tuple[list[str], list[str], list[str]]:
    """
    ...
    if test_dir not in TEST_DIRECTORY_COVERAGE:
        ...
        excluded_dirs.append(f"{test_dir} - Explicitly excluded from coverage (fixtures/data)")
    else:
...
def validate_test_coverage(session: nox.Session) -> None:
    ...
    for test_dir in existing_test_dirs:
        missing_coverage, uncovered_dirs, excluded_dirs = _check_test_directory_coverage(test_dir)
