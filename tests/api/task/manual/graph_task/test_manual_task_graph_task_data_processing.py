...
class TestManualTaskGraphTaskGetSubmittedData:
    """Test the _get_submitted_data method"""

    def test_get_submitted_data_with_no_instances(
        self, task_resources, connection_with_manual_access_task, access_privacy_request
    ):
        """Test _get_submitted_data returns None when no instances exist"""
        graph_task = ManualTaskGraphTask(task_resources)
        _, manual_task, _, _ = connection_with_manual_access_task

        result = graph_task._get_submitted_data(
            manual_task,
    ...
    def test_get_submitted_data_with_incomplete_instance(
        self, task_resources, connection_with_manual_access_task, access_privacy_request
    ):
        """Test _get_submitted_data returns None when instance is incomplete"""
        graph_task = ManualTaskGraphTask(task_resources)
    ...
    def test_get_submitted_data_with_conditional_data(
        ...
        """Test _get_submitted_data merges conditional data correctly"""
        graph_task = ManualTaskGraphTask(task_resources)
        _, manual_task, config, field = connection_with_manual_access_task

        # Create completed instance with submission
        ...
        conditional_data = {"profile": {"age": 25}}
...
class TestManualTaskGraphTaskSetSubmittedDataOrRaise:
    """Test the _set_submitted_data_or_raise_awaiting_async_task_callback method"""

    def test_set_submitted_data_or_raise_with_completed_data(
        self,
        connection_with_manual_access_task,
        access_privacy_request,
        build_graph_task,
    ):
        """Test _set_submitted_data_or_raise_awaiting_async_task_callback with completed data"""
        ...
        # Refresh the privacy request and instance to ensure relationships are loaded
        graph_task.resources.session.refresh(access_privacy_request)
        graph_task.resources.session.refresh(instance)

        result = graph_task._set_submitted_data_or_raise_awaiting_async_task_callback(
            manual_task,
            ManualTaskConfigurationType.access_privacy_request,
            ActionType.access,
        )
    ...
    def test_set_submitted_data_or_raise_with_incomplete_data(
        self,
        connection_with_manual_access_task,
        access_privacy_request,
        build_graph_task,
    ):
        """Test _set_submitted_data_or_raise_awaiting_async_task_callback raises AwaitingAsyncTaskCallback"""
        _, graph_task = build_graph_task
        _, manual_task, config, _ = connection_with_manual_access_task
        ...
        # Verify privacy request status was updated
        graph_task.resources.session.refresh(access_privacy_request)
        assert access_privacy_request.status == PrivacyRequestStatus.requires_input

    def test_set_submitted_data_or_raise_preserves_existing_requires_input_status(
        self,
        connection_with_manual_access_task,
        access_privacy_request,
        build_graph_task,
    ):
        """Test _set_submitted_data_or_raise_awaiting_async_task_callback preserves existing requires_input status"""
        _, graph_task = build_graph_task
        _, manual_task, config, _ = connection_with_manual_access_task

        # Set privacy request to requires_input
        access_privacy_request.status = PrivacyRequestStatus.requires_input
        access_privacy_request.save(graph_task.resources.session)
        ...
        # Verify privacy request status remains requires_input
        graph_task.resources.session.refresh(access_privacy_request)
        assert access_privacy_request.status == PrivacyRequestStatus.requires_input
