...
from fides.api.tasks.storage import (
    convert_dict_to_encrypted_json,
    upload_to_gcs,
    upload_to_local,
    upload_to_s3,
...
class TestWriteToInMemoryBuffer:
    ...
    def test_write_to_in_memory_buffer_manual_webhook_attachments_csv(self):
        ...
        assert isinstance(result, BytesIO)
        with zipfile.ZipFile(result) as zip_file:
            assert "webhook1.csv" in zip_file.namelist()
            assert "webhook2.csv" in zip_file.namelist()
...
class TestConvertToEncryptedJSON:
    def test_convert_dict_to_encrypted_json(self):
        data = {"key": "value"}
        request_id = "test-request-id"

        result = convert_dict_to_encrypted_json(data, request_id)
...
class TestUploadToS3:
    ...
    def test_upload_to_s3_upload_error(
        ...
        # Mock the upload to succeed but presigned URL generation to fail
        s3_client.upload_fileobj = MagicMock()  # This will succeed
        s3_client.generate_presigned_url = MagicMock(
