...
def onesignal_secrets(saas_config):
    return {
        ...
        "player_id": pydash.get(saas_config, "onesignal.player_id") or secrets["player_id"],
    }
...
@pytest.fixture(scope="session")
def onesignal_identity_phone_number(saas_config):
    return (
        pydash.get(saas_config, "onesignal.identity_phone_number") or secrets["identity_phone_number"]
    )
...
def onesignal_create_erasure_data(
    ...
    body = {
     ...
     "test_type": "1",
     "language": "en",
     "external_user_id": "5088124543031",
     "timezone": "-28800",
     "game_version": "1.1.1",
     ...
     "session_count": 600,
     "tags": {
          "first_name": "Ethyca first name",
          "last_name": "Ethyca last name",
          "level": "99",
          "amount_spent": "6000",
          "account_type": "VIP",
          "key_to_delete": "\"\""
    ...
    device_response = requests.post(url=f"{base_url}/api/v1/players", headers=headers, json=body)
    device = device_response.json()['id']
