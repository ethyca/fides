...
class TestSmartOpenStreamingStorageDuplicateFilenames:
    """Test duplicate filename handling in SmartOpenStreamingStorage."""

    def test_get_unique_filename_no_duplicates(self, mock_smart_open_client):
        """Test unique filename generation with no duplicates."""
        storage = SmartOpenStreamingStorage(mock_smart_open_client)

        # First file should keep original name
        result = storage._get_unique_filename("document.pdf")
        assert result == "document.pdf"
        assert "document.pdf" in storage.used_filenames

    def test_get_unique_filename_with_duplicates(self, mock_smart_open_client):
        """Test unique filename generation with duplicates."""
        storage = SmartOpenStreamingStorage(mock_smart_open_client)

        # First file keeps original name
        result1 = storage._get_unique_filename("document.pdf")
        assert result1 == "document.pdf"

        # Second file gets _1 suffix
        result2 = storage._get_unique_filename("document.pdf")
        assert result2 == "document_1.pdf"

        # Third file gets _2 suffix
        result3 = storage._get_unique_filename("document.pdf")
        assert result3 == "document_2.pdf"

        # Verify all names are tracked
        assert "document.pdf" in storage.used_filenames
        assert "document_1.pdf" in storage.used_filenames
        assert "document_2.pdf" in storage.used_filenames

    def test_get_unique_filename_different_extensions(self, mock_smart_open_client):
        """Test unique filename generation with different file extensions."""
        storage = SmartOpenStreamingStorage(mock_smart_open_client)

        # Files with same base name but different extensions should not conflict
        result1 = storage._get_unique_filename("document.pdf")
        result2 = storage._get_unique_filename("document.txt")
        result3 = storage._get_unique_filename("document.docx")

        assert result1 == "document.pdf"
        assert result2 == "document.txt"
        assert result3 == "document.docx"

    def test_get_unique_filename_mixed_scenarios(self, mock_smart_open_client):
        """Test unique filename generation with mixed scenarios."""
        storage = SmartOpenStreamingStorage(mock_smart_open_client)

        # Add some existing files
        storage._get_unique_filename("file1.txt")
        storage._get_unique_filename("file2.pdf")

        # Now test duplicates
        result1 = storage._get_unique_filename("file1.txt")
        result2 = storage._get_unique_filename("file1.txt")
        result3 = storage._get_unique_filename("file2.pdf")

        assert result1 == "file1_1.txt"
        assert result2 == "file1_2.txt"
        assert result3 == "file2_1.pdf"

    def test_used_filenames_reset_on_upload(self, mock_smart_open_client):
        """Test that used filenames are reset at the start of each upload operation."""
        storage = SmartOpenStreamingStorage(mock_smart_open_client)

        # Add some filenames
        storage._get_unique_filename("test.txt")
        storage._get_unique_filename("test.txt")
        assert len(storage.used_filenames) == 2

        # Simulate upload operation start (this would normally be called in upload_to_storage_streaming)
        storage.used_filenames.clear()
        assert len(storage.used_filenames) == 0

        # Now we can use the same filename again
        result = storage._get_unique_filename("test.txt")
        assert result == "test.txt"

    @patch(
        "fides.api.service.storage.streaming.smart_open_streaming_storage.SmartOpenStreamingStorage._create_attachment_content_stream"
    )
    def test_process_attachment_safely_with_duplicate_filenames(
        self, mock_create_stream, mock_smart_open_client
    ):
        """Test that _process_attachment_safely handles duplicate filenames correctly."""
        storage = SmartOpenStreamingStorage(mock_smart_open_client)

        # Mock the content stream
        mock_create_stream.return_value = iter([b"test content"])

        # Create two attachments with the same filename
        attachment1 = AttachmentProcessingInfo(
            attachment=AttachmentInfo(
                storage_key="s3://bucket/path1/document.pdf", file_name="document.pdf"
            ),
            base_path="attachments",
            item={},
        )

        attachment2 = AttachmentProcessingInfo(
            attachment=AttachmentInfo(
                storage_key="s3://bucket/path2/document.pdf", file_name="document.pdf"
            ),
            base_path="attachments",
            item={},
        )
        ...
        result1 = storage._process_attachment_safely(attachment1)
        assert result1[0] == "attachments/document.pdf"  # Original filename

        # Process second attachment
        result2 = storage._process_attachment_safely(attachment2)
        assert result2[0] == "attachments/document_1.pdf"  # Unique filename

        # Verify both filenames are tracked
        assert "document.pdf" in storage.used_filenames
        assert "document_1.pdf" in storage.used_filenames
