...
class TestSmartOpenStorageClient:
    ...
    def test_init_with_s3_storage_type(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            mock_create.assert_called_once_with("s3", storage_secrets)
    ...
    def test_init_with_gcs_storage_type(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            client = SmartOpenStorageClient("gcs", storage_secrets)

            assert client.storage_type == "gcs"
            assert client.storage_secrets == storage_secrets
            assert client._provider_client == mock_provider_client
            mock_create.assert_called_once_with("gcs", storage_secrets)
    ...
    def test_init_with_azure_storage_type(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            client = SmartOpenStorageClient("azure", storage_secrets)

            assert client.storage_type == "azure"
            assert client.storage_secrets == storage_secrets
            assert client._provider_client == mock_provider_client
            mock_create.assert_called_once_with("azure", storage_secrets)
    ...
    def test_abort_multipart_upload(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            # Since abort_multipart_upload is a no-op in SmartOpenStorageClient,
            # we just verify the method exists and can be called without error
            # The actual implementation just logs and doesn't call the provider client

    def test_abort_multipart_upload_with_empty_upload_id(self):
        """Test aborting multipart upload with empty upload ID."""
        storage_secrets = {"test": "secret"}
        bucket = "test-bucket"
        key = "test/file.txt"
        upload_id = ""
    ...
    def test_complete_multipart_upload(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            # Since complete_multipart_upload is a no-op in SmartOpenStorageClient,
            # we just verify the method exists and can be called without error
            # The actual implementation just logs and doesn't call the provider client

    def test_complete_multipart_upload_with_empty_parts(self):
        """Test completing multipart upload with empty parts list."""
        storage_secrets = {"test": "secret"}
        bucket = "test-bucket"
        key = "test/file.txt"
        upload_id = "upload_123"
    ...
    def test_min_part_size_is_class_attribute(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            client1 = SmartOpenStorageClient("s3", storage_secrets)
            client2 = SmartOpenStorageClient("gcs", storage_secrets)
    ...
    def test_storage_type_normalization_edge_cases(self):
        ...
        with patch.object(StorageClientFactory, "create_client") as mock_create:
            ...
            for input_type, expected_type in test_cases:
                client = SmartOpenStorageClient(input_type, storage_secrets)
                assert client.storage_type == expected_type
                mock_create.assert_called_with(input_type, storage_secrets)
