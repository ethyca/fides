...
class TestS3StorageClientObjectOperations:
    ...
    def test_get_object_head_success(self, s3_storage_client, test_file_content):
        ...
        response = s3_storage_client.get_object_head(
            bucket="test-bucket", key="test-head-object.txt"
        )
    ...
    def test_get_object_head_not_exists(self, s3_storage_client):
        ...
        with pytest.raises(ClientError) as exc_info:
            s3_storage_client.get_object_head(
                bucket="test-bucket", key="nonexistent-file.txt"
            )
        ...
        assert any(
            [
                "NoSuchKey" in error_str,  # Real AWS error
                "404" in error_str,  # moto error code
                "Not Found" in error_str,  # moto error message
            ]
        ), f"Expected error to contain NoSuchKey, 404, or Not Found, got: {error_str}"
    ...
    def test_get_object_range_success(self, s3_storage_client, test_file_content):
        ...
        response = s3_storage_client.get_object_range(
            bucket="test-bucket", key="test-range-object.txt", start_byte=0, end_byte=9
        )
    ...
    def test_get_object_range_invalid_range(self, s3_storage_client, test_file_content):
        ...
        assert any(
            [
                "InvalidRange" in error_str,  # Real AWS error
                "416" in error_str,  # moto error code
                "Range Not Satisfiable" in error_str,  # moto error message
            ]
        ), f"Expected error to contain InvalidRange, 416, or Range Not Satisfiable, got: {error_str}"
