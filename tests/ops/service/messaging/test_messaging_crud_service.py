...
from fides.api.schemas.messaging.messaging import MessagingActionType, MessagingTemplateWithPropertiesBodyParams, \
...
from fides.api.service.messaging.messaging_crud_service import (
    ...
    get_messaging_template_by_type, update_messaging_template, create_messaging_template, delete_template_by_id,
    get_template_by_id, get_default_template_by_type, get_all_messaging_templates_summary,
)
...
class TestMessagingTemplates:
    ...
    def test_update_messaging_template(self, db: Session, messaging_template_subject_identity_verification, property_a, property_b):
        update_body = {
            "content": {
                "subject": "Here is your code {{code}}",
                "body": "Use code {{code}} to verify your identity, you have {{minutes}} minutes!",
        ...
        update_messaging_template(db, messaging_template_subject_identity_verification.id, MessagingTemplateWithPropertiesBodyParams(update_body))
    ...
    def test_update_messaging_template_id_not_found(self, db: Session, messaging_template_subject_identity_verification, property_a, property_b):
        update_body = {
            "content": {
                "subject": "Here is your code {{code}}",
                "body": "Use code {{code}} to verify your identity, you have {{minutes}} minutes!",
        ...
        with pytest.raises(MessagingConfigNotFoundException) as exc:
            update_messaging_template(db, "invalid", MessagingTemplateWithPropertiesBodyParams(update_body))

    def test_update_messaging_template_property_not_found(self, db: Session,
    ...
    def test_update_messaging_template_conflicting_template(self, db: Session,
                                                            messaging_template_no_property, messaging_template_subject_identity_verification, property_a):
        update_body = {
            "content": {
                "subject": "Here is your code {{code}}",
                "body": "Use code {{code}} to verify your identity, you have {{minutes}} minutes!",
        ...
        with pytest.raises(Exception) as exc:
            update_messaging_template(db, messaging_template_no_property.id, MessagingTemplateWithPropertiesBodyParams(update_body))

    def test_update_messaging_template_conflicting_template_but_one_disabled(self, db: Session,
                                                            messaging_template_no_property, messaging_template_subject_identity_verification, property_a):
        update_body = {
            "content": {
                "subject": "Here is your code {{code}}",
    ...
    def test_delete_template_by_id(self, db: Session, messaging_template_no_property, messaging_template_subject_identity_verification):
        delete_template_by_id(db, template_id=messaging_template_subject_identity_verification.id)
        messaging_template: Optional[MessagingTemplate] = MessagingTemplate.get(
            db, object_id=messaging_template_subject_identity_verification.id
        )
        messaging_template_to_property: Optional[MessagingTemplateToProperty] = MessagingTemplateToProperty.get(
            db, object_id=messaging_template_subject_identity_verification.id
        )
        assert messaging_template is None
        assert messaging_template_to_property is None

    def test_delete_template_by_id_not_found(self, db: Session, messaging_template_no_property, messaging_template_subject_identity_verification):
        with pytest.raises(MessagingConfigNotFoundException) as exc:
    ...
    def test_delete_template_by_id_cannot_delete_only_type(self, db: Session, messaging_template_subject_identity_verification):
        with pytest.raises(MessagingConfigValidationException) as exc:
            delete_template_by_id(db, template_id=messaging_template_subject_identity_verification.id)
        messaging_template: Optional[MessagingTemplate] = MessagingTemplate.get(
            db, object_id=messaging_template_subject_identity_verification.id
    ...
    def test_get_template_by_id(self, db: Session, messaging_template_subject_identity_verification):
        template = get_template_by_id(db, template_id=messaging_template_subject_identity_verification.id)
    ...
    def test_get_default_template_by_type(self, db: Session):
        default: MessagingTemplateWithPropertiesDetail = get_default_template_by_type(MessagingActionType.SUBJECT_IDENTITY_VERIFICATION.value)
    ...
    def test_get_all_messaging_templates_summary_some_db_templates(self, db: Session, messaging_template_subject_identity_verification, messaging_template_privacy_request_receipt):
        summary = get_all_messaging_templates_summary(db)
        assert len(summary) == 6

    def test_get_all_messaging_templates_summary_all_db_templates(self, db: Session, property_a):
        content = {
