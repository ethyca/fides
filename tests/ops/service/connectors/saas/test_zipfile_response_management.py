...
from fides.api.schemas.saas.shared_schemas import SaaSRequestParams, HTTPMethod
    ...
    def do_GET(self):
        ...
        if self.path == "/test.zip":
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                zip_file.writestr("binary.dat", b'\xe1\x93\x5e\x5b\x00\x01\x02\x03\xff\xfe')
...
class TestZipFileResponseManagement:
    ...
    @pytest.fixture
    def authenticated_client(self, test_connection_config, test_client_config, zip_test_server):
    ...
    def test_valid_zip_file_download(self, authenticated_client):
        ...
        assert response.content.startswith(b'PK'), "Content should start with ZIP signature"
        ...
        assert "invalid continuation byte" in str(exc_info.value) or "invalid start byte" in str(exc_info.value)
    ...
    def test_corrupted_zip_file_download(self, authenticated_client):
        ...
        assert problematic_bytes == b'\xe1\x93', f"Expected problematic bytes at position 10-11, got {problematic_bytes.hex()}"
        ...
        assert error.start == 10, f"Expected UTF-8 error at position 10, got {error.start}"
        assert error.end == 12, f"Expected UTF-8 error end at position 12, got {error.end}"
