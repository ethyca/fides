...
class TestRecurlyConnector:
    ...
    async def test_non_strict_erasure_request(
        ...
        assert account['data'][0]["first_name"] == "MASKED"
        assert account['data'][0]["last_name"]  == "MASKED"

        address_response = recurly_client.get_shipping_address(account_id)
        address_data = address_response.json()
        assert address_data['data'][0]["first_name"] == "MASKED"
        assert address_data['data'][0]["last_name"]  == "MASKED"

        billing_response = recurly_client.get_billing_info(account_id)
        billing_data = billing_response.json()
        assert billing_data["first_name"] == "MASKED"
        assert billing_data["last_name"]  == "MASKED"