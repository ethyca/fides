...
async def test_shippo_access_request_task(
    ...
    for ticket in v[f"{dataset_name}:address"]:
        assert ticket["object_owner"] == shippo_identity_email

    for ticket_comment in v[f"{dataset_name}:orders"]:
        assert ticket_comment["object_owner"] == shippo_identity_email