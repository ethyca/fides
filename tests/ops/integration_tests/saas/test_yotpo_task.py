...
async def test_yotpo_access_request_task(
    ...
    v = await graph_task.run_access_request(
        ...
        graph,
        [yotpo_connection_config],
        # {"email": yotpo_identity_email,"app_key":yotpo_app_key,"secret_key":yotpo_secret_key},
    ...
    # assert v[f"{dataset_name}:get_utoken_(core)"][0]["email"] == yotpo_identity_email
    access_token = v[f"{dataset_name}:get_utoken_(core)"][0]["access_token"]
...
async def test_yotpo_erasure_request_task(
    ...
    # user
    customer_responses = requests.get(
