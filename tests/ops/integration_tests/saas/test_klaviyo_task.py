...
async def test_klaviyo_access_request_task(
    ...
    assert v[f"{dataset_name}:profile"][0]['attributes']["email"] == klaviyo_identity_email
    user_id = v[f"{dataset_name}:profile"][0]["id"]
...
async def test_klaviyo_erasure_request_task(
    ...
    # Since user is deleted, it won't be available so response is 200
