...
class TestConfigProxy:
    ...
    @pytest.mark.usefixtures("insert_app_config")
    def test_execution_finalization_config_proxy(self, db):
        """Verify execution finalization config proxy works as expected"""
        config_proxy = ConfigProxy(db)
        assert config_proxy.execution.erasure_request_finalization_required is False
        assert config_proxy.execution.consent_request_finalization_required is False

        # Override via API for erasure request finalization
        ApplicationConfig.update_api_set(
            db,
            api_set_dict={"execution": {"erasure_request_finalization_required": True}},
        )
        assert config_proxy.execution.erasure_request_finalization_required is True
        assert config_proxy.execution.consent_request_finalization_required is False

        # Override via API for consent request finalization
        ApplicationConfig.update_api_set(
            db,
            api_set_dict={"execution": {"consent_request_finalization_required": True}},
        )
        assert config_proxy.execution.erasure_request_finalization_required is True
        assert config_proxy.execution.consent_request_finalization_required is True

        # Unset via API for erasure request finalization, should revert to config-set value
        ApplicationConfig.update_api_set(
            db,
            api_set_dict={"execution": {"erasure_request_finalization_required": None}},
        )
        assert config_proxy.execution.erasure_request_finalization_required is False
        assert config_proxy.execution.consent_request_finalization_required is True

        # Unset via API for consent request finalization, should revert to config-set value
        ApplicationConfig.update_api_set(
            db,
            api_set_dict={"execution": {"consent_request_finalization_required": None}},
        )
        assert config_proxy.execution.erasure_request_finalization_required is False
        assert config_proxy.execution.consent_request_finalization_required is False
