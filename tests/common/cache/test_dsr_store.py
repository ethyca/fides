...
def _run_dsr_store_assertions() -> None:
    """Run the same tests without pytest (no DB/Redis). Used by __main__."""
    def fresh() -> tuple[InMemoryRedisCacheManager, DSRCacheStore]:
        m = InMemoryRedisCacheManager()
        return m, DSRCacheStore(m)

    t = TestDSRCacheStoreWithInMemoryManager()
    m1, s1 = fresh()
    t.test_set_and_get(s1)
    m2, s2 = fresh()
    t.test_get_missing_returns_none(s2)
    m3, s3 = fresh()
    t.test_set_with_index_registers_key_in_index(s3, m3)
    m4, s4 = fresh()
    t.test_get_all_keys_returns_indexed_keys(s4, m4)
    m5, s5 = fresh()
    t.test_clear_removes_all_keys_and_index(s5, m5)
    m6, s6 = fresh()
    t.test_delete_removes_key_and_index_entry(s6, m6)
    m7, s7 = fresh()
    t.test_get_with_legacy_reads_new_key_first(s7, m7)
    m8, s8 = fresh()
    t.test_get_with_legacy_migrates_from_legacy_key(s8, m8)
    _, s9 = fresh()
    t.test_write_custom_field_and_get_custom_field(s9)
    _, s10 = fresh()
    t.test_convenience_async_execution(s10)
    m11, s11 = fresh()
    t.test_retry_count(s11, m11)
    m12, s12 = fresh()
    t.test_drp(s12, m12)
    m13, s13 = fresh()
    t.test_encryption(s13, m13)
    m14, s14 = fresh()
    t.test_masking_secret(s14, m14)
    print("All DSR store assertions passed.")


if __name__ == "__main__":
    _run_dsr_store_assertions()
