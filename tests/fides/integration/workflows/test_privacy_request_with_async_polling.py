...
from tests.fides.conftest import wait_for_privacy_request_status
from tests.fides.ops.test_helpers.saas_test_utils import MockAuthenticatedClient
...
class TestPrivacyRequestWithAsyncPolling:
    ...
    def async_polling_connector(self, db: Session) -> Generator:
        with open(
            "tests/fides/integration/workflows/data/example_async_polling_config.yml",
            "r",
            encoding="utf-8",
        ) as saas_config_file:
        ...
        with open(
            "tests/fides/integration/workflows/data/example_async_polling_dataset.yml",
            "r",
            encoding="utf-8",
        ) as dataset_file:
    ...
    def async_polling_error_connector(self, db: Session) -> Generator:
        ...
        """
        with open(
            "tests/fides/integration/workflows/data/example_async_polling_error_config.yml",
            "r",
            encoding="utf-8",
        ) as saas_config_file:
            saas_config = yaml.safe_load(saas_config_file)["saas_config"]
        ...
        with open(
            "tests/fides/integration/workflows/data/example_async_polling_error_dataset.yml",
            "r",
            encoding="utf-8",
        ) as dataset_file:
            dataset = yaml.safe_load(dataset_file)["dataset"][0]
