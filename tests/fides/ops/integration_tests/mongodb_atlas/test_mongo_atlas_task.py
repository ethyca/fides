...
from tests.fides.ops.graph.graph_test_util import assert_rows_match, erasure_policy, field
...
async def test_composite_key_erasure_atlas(
    ...
    # Test erasure with composite keys
    erasure_results = erasure_runner_tester(
        privacy_request_with_erasure_policy,
        policy,
        DatasetGraph(dataset),
...
async def test_object_querying_mongo_atlas(
    ...
    access_request_results = access_runner_tester(
        ...
        policy,
        dataset_graph,
        [postgres_config, integration_mongodb_atlas_config],
        {"email": "customer-1@example.com"},
        db,
    )
...
async def test_return_all_elements_config_erasure_atlas(
    ...
    seed_email = postgres_inserts["customer"][0]["email"]
    seed_phone = mongodb_atlas_inserts["rewards"][0]["owner"][0]["phone"]
...
@pytest.mark.integration_mongodb_atlas
@pytest.mark.integration_external
class TestRetrievingDataMongoDBAtlas:
    """Test data retrieval operations specific to MongoDB Atlas"""

    @pytest.fixture
    def connector(self, integration_mongodb_atlas_config):
        return get_connector(integration_mongodb_atlas_config)

    @pytest.fixture(autouse=True)
    def seeded_data(self, mongodb_atlas_inserts):
        """Auto-use fixture to ensure data is seeded for all tests in this class"""
    ...
    def test_mongodb_atlas_connector_properties(
        self, connector, integration_mongodb_atlas_config
    ):
        """Test that Atlas connector has correct configuration properties"""
        # Verify connector is properly configured for Atlas
        uri = connector.build_uri()
        assert uri.startswith("mongodb+srv://"), "Should use SRV scheme for Atlas"
        assert "ssl=true" in uri, "Should enable SSL for Atlas"
