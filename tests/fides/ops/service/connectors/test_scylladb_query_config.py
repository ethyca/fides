...
class TestScyllaDBQueryConfig:
    ...
    def test_dry_run_query_with_data(self, complete_execution_node):
        ...
        assert (
            dry_run_query
            == "SELECT age, alternative_contacts, do_not_contact, double_data, email, float_data, last_contacted, name, states_lived, user_id, uuid FROM users WHERE email = ? ALLOW FILTERING;"
        )
