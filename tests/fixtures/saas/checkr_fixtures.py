...
def checkr_secrets(saas_config) -> Dict[str, Any]:
    return {
        "domain": pydash.get(saas_config, "checkr.domain")
        or secrets["domain"],
...
def checkr_identity_email(saas_config) -> str:
    return (
        pydash.get(saas_config, "checkr.identity_email") or secrets["identity_email"]
    )
...
@pytest.fixture
def checkr_external_references() -> Dict[str, Any]:
    return {}


@pytest.fixture
def checkr_erasure_external_references() -> Dict[str, Any]:
    return {}
...
def checkr_runner(
    ...
    cache,
    checkr_secrets,
    checkr_external_references,
    checkr_erasure_external_references,
) -> ConnectorRunner:
    return ConnectorRunner(
        ...
        "checkr",
        checkr_secrets,
        external_references=checkr_external_references,
        erasure_external_references=checkr_erasure_external_references,
    )
