...
def google_cloud_sql_postgres_connection_config(db: Session) -> Generator:
    connection_config = ConnectionConfig.create(
        db=db,
        data={
            "name": str(uuid4()),
            "key": "my_google_cloud_postgres_config",
            "connection_type": ConnectionType.google_cloud_sql_postgres,
            "access": AccessLevel.write,
        },
    )
    # Pulling from integration config file or GitHub secrets
    google_cloud_sql_postgres_integration_config = integration_config.get(
        "google_cloud_sql_postgres", {}
    )
    db_iam_user = google_cloud_sql_postgres_integration_config.get(
        "db_iam_user"
    ) or os.environ.get("GOOGLE_CLOUD_SQL_POSTGRES_DB_IAM_USER")

    instance_connection_name = google_cloud_sql_postgres_integration_config.get(
...
@pytest.fixture(scope="function")
def google_cloud_sql_postgres_integration_session(
    google_cloud_sql_postgres_connection_config,
):
    engine = GoogleCloudSQLPostgresConnector(
        google_cloud_sql_postgres_connection_config
    ).client()
...
def google_cloud_sql_postgres_integration_db(
    ...
    """
    yield google_cloud_sql_postgres_integration_session


@pytest.fixture
def google_cloud_sql_postgres_example_test_dataset_config(
    google_cloud_sql_postgres_connection_config: ConnectionConfig,
    db: Session,
    ...
    google_cloud_sql_postgres_connection_config.save(db=db)
    ...
    dataset = DatasetConfig.create(
        db=db,
        data={
            "connection_config_id": google_cloud_sql_postgres_connection_config.id,
            "fides_key": fides_key,
