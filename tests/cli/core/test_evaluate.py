...
from fides.cli.core import evaluate
...
def test_get_evaluation_policies_with_key_found_local() -> None:
    ...
    with patch("fides.cli.core.evaluate.get_server_resource", get_server_resource_mock):
...
def test_get_evaluation_policies_with_no_key(test_config: FidesConfig) -> None:
    ...
    with patch(
        "fides.cli.core.evaluate.get_all_server_policies",
...
def test_failed_evaluation_error_message(
  ...
  'violations': [ { 'detail': 'Declaration (Share Political Opinions) of '
    ...
    with pytest.raises(SystemExit):
        evaluate.evaluate(
            url=test_config.cli.server_url,
            manifests_dir="tests/cli/data/failing_declaration_taxonomy.yml",
            headers=test_config.user.auth_header,
            local=True,
        )
