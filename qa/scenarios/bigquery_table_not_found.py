...
class BigQueryTableNotFound(QATestScenario):
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            self.step(
                2,
                f"Creating modified dataset with renamed '{self.target_table}' collection",
            )
    ...
    def _create_modified_dataset(self) -> Dict[str, Any]:
        ...
        if not original_dataset_path.exists():
            raise FileNotFoundError(
                f"Original dataset file not found: {original_dataset_path}"
            )
        ...
        try:
            ...
            if "dataset" not in original_data or not isinstance(
                original_data["dataset"], list
            ):
                raise ValueError(
            ...
            for collection in original_dataset.get("collections", []):
                if collection["name"] == self.target_table:
                    ...
                    self.info(
                        f"Renamed collection '{self.target_table}' to '{target_table_missing}'"
                    )
                else:
            ...
            self.success(
                f"Created modified dataset with {len(modified_dataset['collections'])} collections"
            )
            self.info(
                f"Updated all internal dataset references from 'bigquery_example_test_dataset' to '{self.dataset_key}'"
            )
            if self.target_table != target_table_missing:
                self.info(
                    f"Updated any field references from '{self.target_table}.*' to '{target_table_missing}.*' in other collections"
                )
            self.warning(
        ...
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                if key == "dataset" and value == old_dataset:
                    ...
                    and value.startswith(f"{old_table}.")
                ):
                    ...
                    result[key] = self._deep_copy_and_update_references(
                        value, old_dataset, new_dataset, old_table, new_table
                    )
            return result
        elif isinstance(obj, list):
            return [
                self._deep_copy_and_update_references(
                    item, old_dataset, new_dataset, old_table, new_table
                )
                for item in obj
            ]
        else:
