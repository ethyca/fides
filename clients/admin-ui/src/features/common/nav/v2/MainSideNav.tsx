...
export const UnconnectedMainSideNav = ({
  ...
  <Box p={4} minWidth="200px" maxWidth="200px" backgroundColor="#191D27" height={"100%"} overflow={"scroll"}>
    <VStack as="nav" alignItems="start" color="white" height={"100%"} justifyContent={"space-between"}>
      <Box>
        <Box pb={6}>
          <FidesLogoHomeLink />
        </Box>
          ...
          <Menu>
            ...
            <MenuList shadow="xl" zIndex="20">
              <Stack px={3} py={2} spacing={1}>
                <Text color={"gray.700"} fontWeight="medium">{username.username}</Text>
              </Stack>
...
const MainSideNav = () => {
  ...
  const username = useAppSelector(selectUser) ?? { username: "" }; // Define username here

  const handleLogout = async () => {
    await logoutMutation({});
    dispatch(logout());
  };
  return <UnconnectedMainSideNav {...nav} handleLogout={handleLogout} username={username} />;
};
