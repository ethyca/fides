import { Box, Button, Stack, useToast, Text, ButtonGroup, Heading } from "@fidesui/react";
...
const LoadWebScanner = () => {
  ...
  const skip = true;
  const {isSuccess,isLoading, isError, error, data}= useGetWebScanQuery(initialValues, {
    skip
  });
  ...
  const handleError = (requestError: RTKErrorResult["error"]) => {
    const parsedError = parseError(requestError, {
      status: 500,
      message:
        "Our system encountered a problem while scanning your website.",
    });
  ...
  return (
    <Box id="web-scanner-form" display="flex" justifyContent="center" width="100%" >
      <Formik initialValues={initialValues} onSubmit={()=>{}} validationSchema={validationSchema}>
        {({isSubmitting, dirty, isValid})=>(
          ...
          <Form>
          <Heading as="h3" size="lg" fontWeight="semibold" mb={10}>Website Scan</Heading>
          <Text mb={7}>Provide a target website(e.g. "https://ethyca.com"), and
            Fides will automatically scanand generate a list of systems
            based on the web requests detected in the browser.</Text>
            <Box mb={5}>
