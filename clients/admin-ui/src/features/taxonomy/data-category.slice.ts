...
const dataCategoryApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
...
export const { setIsAddFormOpen } = dataCategorySlice.actions;

const emptyDataCategories: DataCategory[] = [];
export const selectDataCategories: (state: RootState) => DataCategory[] =
  createSelector(
    dataCategoryApi.endpoints.getAllDataCategories.select(),
...
const selectDataCategoryState = (state: RootState) => state.dataCategory;

export const selectIsAddFormOpen = createSelector(
  selectDataCategoryState,
  (state) => state.isAddFormOpen,
);

export const { reducer } = dataCategorySlice;
