...
const isIDP = (resourceType?: StagedResourceTypeValue): boolean =>
  resourceType === StagedResourceTypeValue.OKTA_APP;

const buildIdpColumns = ({
  rowClickUrl,
}: Pick<
  ColumnBuilderParams,
  "rowClickUrl"
...
export const useDiscoveredSystemAggregateColumns = ({
  ...
  const columns: ColumnsType<SystemStagedResourcesAggregateRecord> =
    useMemo(() => {
      if (isIDP(resourceType)) {
        return buildIdpColumns({ rowClickUrl });
      }
