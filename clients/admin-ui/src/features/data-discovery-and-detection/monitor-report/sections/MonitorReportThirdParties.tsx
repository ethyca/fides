...
export const MonitorReportsThirdParties = ({
  ...
  <>
    <Text>{`All ${vendors.length} vendor systems identified by Fides, categorized against our internal vendor taxonomy. Data shared columns pair a human-readable label from Fides. Data use is what drives the policy enforcement in the platform`}</Text>
    {vendorsChart && (
      <View
        style={{
          flexDirection: "row",
    ...
    <PdfTable
      data={vendors.map(({ name, location, resource_count, data_uses }) => ({
        key: name,
        name: (
          <View
            style={{
