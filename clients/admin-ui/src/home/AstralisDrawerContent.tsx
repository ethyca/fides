...
const MOCK_CONVERSATIONS = [
  {
    steward: "Casey Martinez",
    system: "Snowflake Analytics",
    status: "in_progress" as const,
    days: 2,
  },
  {
    steward: "Jamie Torres",
    system: "BigQuery Prod",
    status: "in_progress" as const,
    days: 1,
  },
  {
    ...
    <Flex vertical gap="middle">
      ...
      <Flex vertical gap={0}>
        {MOCK_CONVERSATIONS.map((conv) => {
          ...
          const isOverdue = conv.status === "awaiting" && conv.days > 3;

          return (
            <Flex
              key={`${conv.steward}-${conv.system}`}
              justify="space-between"
              align="center"
              className="border-b border-solid border-b-[var(--ant-color-border)] py-3"
            >
              <Flex vertical gap={2}>
                <Text strong>{conv.steward}</Text>
                <Text type="secondary" className="text-xs">
                  {conv.system}
                </Text>
              </Flex>
