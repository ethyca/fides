...
describe("Privacy Requests", () => {
  ...
  describe("Access Package tab", () => {
    ...
    beforeEach(() => {
      stubPlus(true);
      cy.overrideFeatureFlag("accessPackages", true);
    ...
    describe("when the flag is disabled", () => {
      beforeEach(() => {
        cy.overrideFeatureFlag("accessPackages", false);
        stubPrivacyRequests(PrivacyRequestStatus.AWAITING_ACCESS_REVIEW);
        visitDetailPage();
      });

      it("does not show the Access package tab", () => {
        cy.contains(".ant-tabs-tab", "Access package").should("not.exist");
      });
    });
    ...
    describe("when the request is awaiting access review", () => {
      ...
      it("approves the package and shows a success message", () => {
      ...
      it("enables the Approve button when awaiting review", () => {
