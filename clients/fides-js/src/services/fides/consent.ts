...
import {PrivacyExperience, PrivacyPreferencesCreateWithCode} from "~/lib/consent-types";
...
export const getExperience = (userLocationString: String, debug: boolean): PrivacyExperience | undefined => {
    debugLog(
        debug,
        "Fetching experience for location...",
        userLocationString
    );
 ...
 */
export const saveUserPreference = (userDeviceId: String, debug: boolean, preferences: PrivacyPreferencesCreateWithCode): PrivacyExperience | undefined => {
    debugLog(
        debug,
