...
class MailchimpTransactionalService(BaseEmailProviderService):
    provider_name: Literal["Mailchimp Transactional"] = "Mailchimp Transactional"

    def __init__(self, messaging_config):
        super().__init__(messaging_config)

        self.messaging_config_details = (
            MessagingServiceDetailsMailchimpTransactional.model_validate(
                messaging_config.details
            )
        )
        self.messaging_config_secrets = (
            MessagingServiceSecretsMailchimpTransactional.model_validate(
                messaging_config.secrets
            )
        )

    def send_email(self, to: str, message: EmailForActionType) -> None:
        from_email = self.messaging_config_details.email_from
        data = json.dumps(
            {
                "key": self.messaging_config_secrets.mailchimp_transactional_api_key,
                "message": {
