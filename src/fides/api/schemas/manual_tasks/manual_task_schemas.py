...
class ManualTaskParentEntityType(str, Enum):
    ...
    CONNECTION_CONFIG = "connection_config"  # used for access and erasure privacy requests
...
class ManualTaskFieldMetadata(FidesSchema):
    ...
    help_text: Optional[str] = Field(None, description="Help text to display with the field")
...
class ManualTaskFormFieldMetadata(ManualTaskFieldMetadata):
    ...
    validation_rules: Optional[Dict[str, Any]] = Field(None, description="Validation rules for the field")
...
class ManualTaskCheckboxFieldMetadata(ManualTaskFieldMetadata):
    ...
    default_value: Optional[bool] = Field(False, description="Default value for the checkbox")
...
class ManualTaskAttachmentFieldMetadata(ManualTaskFieldMetadata):
    ...
    file_types: List[str] = Field(..., description="Allowed file types for upload fields")
    max_file_size: int = Field(..., description="Maximum file size in bytes for upload fields")
    multiple: bool = Field(default=False, description="Whether multiple files can be uploaded")
    max_files: Optional[int] = Field(None, description="Maximum number of files allowed when multiple is true")
...
class ManualTaskFormField(FidesSchema):
    ...
    field_type: ManualTaskFieldType = Field(ManualTaskFieldType.form, description="Type of the field")
    metadata: ManualTaskFormFieldMetadata = Field(..., description="Field metadata and configuration")
...
class ManualTaskCheckboxField(FidesSchema):
    ...
    field_key: str = Field(..., description="Unique key for the field")
    field_type: ManualTaskFieldType = Field(ManualTaskFieldType.checkbox, description="Type of the field")
    metadata: ManualTaskCheckboxFieldMetadata = Field(..., description="Field metadata and configuration")
...
class ManualTaskAttachmentField(FidesSchema):
    ...
    field_key: str = Field(..., description="Unique key for the field")
    field_type: ManualTaskFieldType = Field(ManualTaskFieldType.attachment, description="Type of the field")
    metadata: ManualTaskAttachmentFieldMetadata = Field(..., description="Field metadata and configuration")


# Union type for all field types
ManualTaskField = Union[ManualTaskFormField, ManualTaskCheckboxField, ManualTaskAttachmentField]
...
class ManualTaskConfigCreate(FidesSchema):
    ...
    task_id: str = Field(..., description="ID of the task this configuration belongs to")
    config_type: ManualTaskConfigurationType = Field(..., description="Type of configuration")
...
class ManualTaskSubmissionResponse(FidesSchema):
    ...
    completed_by_id: Optional[str] = Field(None, description="ID of the user who completed")
