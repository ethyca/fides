...
def start_webserver(port: int = 8080) -> None:
    ...
    server = Server(Config(app, host="0.0.0.0", port=port, reload=True, reload_dirs=["/fidesplus/src"], log_level=WARNING))
    ...
    logger.info(f"reload_dirs: {server.config.reload_dirs}, reload: {server.config.reload}")
