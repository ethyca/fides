...
def get_has_identity_cache_expired() -> bool:
    ...
    cache: FidesopsRedis = get_cache()
    result = cache.get(DECRYPTED_IDENTITY_CACHE_KEY)
    if result:
        return False
    return True
...
def build_decrypted_identities_automaton(query: Query) -> ahocorasick.Automaton:  # type: ignore
    ...
    if get_has_identity_cache_expired():
        logger.info(
            "Decrypted identity cache does not exist or has expired. Proceeding to cache all decrypted identities..."
        )
        ...
        for request in all_privacy_requests:
            all_identities_for_request: Optional[Dict[str, Any]] = (
                request.retrieve_decrypted_identities_from_cache_by_privacy_request()
            )
            if not all_identities_for_request:
                # Safeguard if specific privacy requests do not have identities stored in the cache
                logger.info(
                    "Decrypted identities could not be found in cache for privacy request. Proceeding to look up in DB instead of using cache."
                )
                _add_decrypted_identities_to_automaton(request.get_persisted_identity.__dict__, request.id, automaton)  # type: ignore
                # Write to cache for later
                request.cache_decrypted_identities_by_privacy_request()
            _add_decrypted_identities_to_automaton(all_identities_for_request, request.id, automaton)  # type: ignore
    return automaton
