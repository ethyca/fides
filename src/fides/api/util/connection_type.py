...
def get_connection_type_secret_schema(*, connection_type: str) -> dict[str, Any]:
    ...
    if connection_type in [db_type.value for db_type in ConnectionType]:
        ...
        if total_ms > 10:
            logger.warning(
                "PERF get_connection_type_secret_schema: type={} total={:.1f}ms (get_connection_types={:.1f}ms, path=db_type)",
                connection_type,
                total_ms,
                t_get_types_ms,
            )
    ...
    transform_v2_to_v1_in_place(schema)
    t_schema_gen_ms = (time.perf_counter() - t_schema_gen) * 1000
    ...
    hidden_props = [
        name
        for name, attrs in schema["properties"].items()
    ...
    if total_ms > 10:
        logger.warning(
            "PERF get_connection_type_secret_schema: type={} total={:.1f}ms "
            "(get_connection_types={:.1f}ms, template_lookup={:.1f}ms, schema_gen={:.1f}ms)",
            connection_type,
            total_ms,
            t_get_types_ms,
            t_template_ms,
...
def get_saas_connection_types(
    ...
    t_types_ms = (time.perf_counter() - t_types) * 1000
    ...
    if total_ms > 10:
        logger.warning(
            "PERF get_saas_connection_types: total={:.1f}ms (enumerate={:.1f}ms, build_map={:.1f}ms, {} types)",
            total_ms,
            t_types_ms,
            t_build_ms,
            len(saas_types),
