...
connect_args["server_settings"] = {
    "jit": "off",
    # Set default transaction isolation level to avoid explicit BEGIN ISOLATION LEVEL statements
    # READ COMMITTED is PostgreSQL's default, but setting it here ensures consistency
    # and may help avoid the overhead of explicit isolation level setting per transaction
...
async def warmup_async_pool() -> None:
    ...
    pool_size = CONFIG.database.api_async_engine_pool_size
    if pool_size <= 0:
        logger.debug("Skipping async pool warmup: pool_size is 0 or negative")
        return
