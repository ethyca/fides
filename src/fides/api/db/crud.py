...
async def create_resource(
    ...
    log.bind(sql_model=sql_model.__name__, fides_key=resource_dict["fides_key"])
    try:
...
async def get_resource(
    ...
    """
    log.bind(sql_model=sql_model.__name__, fides_key=fides_key)
    async with async_session.begin():
        try:
            log.debug("Fetching resource")
...
async def list_resource(sql_model: Base, async_session: AsyncSession) -> List[Base]:
    ...
    """
    log.bind(sql_model=sql_model.__name__)
    async with async_session.begin():
        try:
