...
def _create_execution_logs_for_skipped_email_send(
    ...
    """Create skipped execution logs for relevant connectors if this privacy request does not need an email send"""
    for connection_config in can_skip_erasure_email:
        ExecutionLog.create(
            db=db,
            data={
                "connection_key": connection_config.key,
                "dataset_name": connection_config.name,
                "collection_name": connection_config.name,
                "privacy_request_id": privacy_request.id,
                "action_type": ActionType.erasure,
                "status": ExecutionLogStatus.skipped,
                "message": f"Erasure email skipped for '{connection_config.name}'",
            },
        )
    ...
    for connection_config in can_skip_consent_email:
        ExecutionLog.create(
            ...
            data={
                ...
                "action_type": ActionType.consent,
                "status": ExecutionLogStatus.skipped,
                "message": f"Consent email skipped for '{connection_config.name}'",
            },
        )
