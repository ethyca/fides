...
JSONB array. This allows customers to customize GPC defaults for new experiences
without having those customizations overwritten on app startup.
...
IMPORTANT: These translations use the EXACT same strings as the experiencetranslation
migration (f8a9b0c1d2e3) to ensure consistency.
"""
...
# GPC translations - EXACT same as in f8a9b0c1d2e3_add_gpc_translation_fields.py
# fmt: off
GPC_TRANSLATIONS = {
    "ar": {
...
def upgrade():
    """
    Add GPC fields to each translation object in experienceconfigtemplate.translations JSONB array.
    This is done per-template, per-language to apply the correct localized GPC strings.
    """
    connection = op.get_bind()

    # Get all templates
    result = connection.execute(
        sa.text("SELECT id, translations FROM experienceconfigtemplate WHERE translations IS NOT NULL")
    )
    templates = [(row[0], row[1]) for row in result]

    for template_id, translations in templates:
        if not translations:
            continue
        ...
        for translation in translations:
            ...
            gpc_values = GPC_TRANSLATIONS.get(language, EN_FALLBACK)

            # Add GPC fields to the translation
            updated_translation = {
                **translation,
                "gpc_label": gpc_values["gpc_label"],
                "gpc_description": gpc_values["gpc_description"],
                "gpc_status_applied_label": gpc_values["gpc_status_applied_label"],
                "gpc_status_overridden_label": gpc_values["gpc_status_overridden_label"],
                "gpc_title": gpc_values["gpc_title"],
            }
            updated_translations.append(updated_translation)

        # Update the template with the new translations
        import json
        connection.execute(
            sa.text(
                """
                UPDATE experienceconfigtemplate
                SET translations = :translations::jsonb[]
                WHERE id = :id
                """
            ),
            {"id": template_id, "translations": updated_translations},
        )
        logger.info(
            f"Updated template '{template_id}' with GPC fields for {len(updated_translations)} translation(s)"
        )


def downgrade():
    """
    Remove GPC fields from experienceconfigtemplate.translations JSONB array.
    """
    connection = op.get_bind()
    ...
    gpc_keys = ["gpc_label", "gpc_description", "gpc_status_applied_label", "gpc_status_overridden_label", "gpc_title"]
    ...
    for template_id, translations in templates:
        ...
        for translation in translations:
            # Remove GPC fields
            updated_translation = {k: v for k, v in translation.items() if k not in gpc_keys}
            updated_translations.append(updated_translation)
        ...
        logger.info(f"Removed GPC fields from template '{template_id}'")
