...
Revision ID: bf08faedf508
Revises: 7799269e7078
Create Date: 2024-03-24 20:38:15.960100
...
def upgrade():
    ...
    sa.Column('upstream_tasks', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    ...
    sa.Column('access_data', sqlalchemy_utils.types.encrypted.encrypted_type.StringEncryptedType(), nullable=True),
    sa.Column('data_for_erasures', sqlalchemy_utils.types.encrypted.encrypted_type.StringEncryptedType(), nullable=True),
    sa.Column('erasure_input_data', sqlalchemy_utils.types.encrypted.encrypted_type.StringEncryptedType(), nullable=True),
    ...
    sa.ForeignKeyConstraint(['privacy_request_id'], ['privacyrequest.id'], ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_requesttask_action_type'), 'requesttask', ['action_type'], unique=False)
    op.create_index(op.f('ix_requesttask_collection_address'), 'requesttask', ['collection_address'], unique=False)
    op.create_index(op.f('ix_requesttask_collection_name'), 'requesttask', ['collection_name'], unique=False)
    op.create_index(op.f('ix_requesttask_dataset_name'), 'requesttask', ['dataset_name'], unique=False)
    op.create_index(op.f('ix_requesttask_id'), 'requesttask', ['id'], unique=False)
    op.create_index(op.f('ix_requesttask_privacy_request_id'), 'requesttask', ['privacy_request_id'], unique=False)
    op.create_index(op.f('ix_requesttask_status'), 'requesttask', ['status'], unique=False)
    op.add_column('privacyrequest', sa.Column('filtered_final_upload', sqlalchemy_utils.types.encrypted.encrypted_type.StringEncryptedType(), nullable=True))
