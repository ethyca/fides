...
from fides.api.task.manual.manual_task_utils import ManualTaskAddress, get_manual_tasks_for_connection_config
...
class ManualTaskGraphTask(GraphTask):
    ...
    def access_request(self, *inputs: List[Row]) -> List[Row]:
        ...
        submitted_data = self._get_submitted_data(db, manual_tasks, self.resources.request)
    ...
    def _get_submitted_data(
        ...
        for manual_task in manual_tasks:
            for config in manual_task.configs:
                ...
                instance = db.query(ManualTaskInstance).filter(
                    ManualTaskInstance.task_id == manual_task.id,
                    ManualTaskInstance.config_id == config.id,
                    ManualTaskInstance.entity_id == privacy_request.id,
                    ManualTaskInstance.entity_type == ManualTaskEntityType.privacy_request
                ...
                required_fields = [field for field in config.field_definitions if field.field_metadata.get('required', False)]
                ...
                submitted_field_ids = {submission.field_id for submission in submissions}
                ...
                # Group by task name or ID
                task_key = manual_task.name or f"task_{manual_task.id}"
