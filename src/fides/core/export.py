...
def generate_system_records(  # pylint: disable=too-many-nested-blocks, too-many-branches, too-many-statements
    ...
    custom_columns = {}
    formatted_data_uses, custom_data_use_keys = format_data_uses(
        server_resources["data_use"]
    )
    formatted_data_subjects, custom_data_subject_keys = format_data_subjects(
    ...
    data_use_custom_field_headers = []  # pylint: disable=unused-variable
    ...
    # but eventually we may want to include these!
    data_subject_custom_field_headers = []  # pylint: disable=unused-variable
    ...
    for system in server_resources["system"]:
        ...
        if system.get("privacy_declarations"):
            for declaration in system["privacy_declarations"]:
                declaration = formatted_privacy_declarations[
                    declaration["id"]
