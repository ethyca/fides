...
class LoggingSettings(FidesSettings):
    ...
    # Logging
    destination: str = Field(default="", description="The output location for log files. Accepts any valid file path. If left unset, log entries are printed to stdout and log files are not produced.")
    level: str = Field(default="INFO", description="The minimum log entry level to produce. Also accepts TRACE, DEBUG, WARNING, ERROR, or CRITICAL (case insensitive).")
    serialization: str = Field(default="", description="The format with which to produce log entries. If left unset, produces log entries formatted using the internal custom formatter. Also accepts 'JSON' (case insensitive).")
    log_pii: bool = Field(
        default=False,
